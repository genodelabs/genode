                          ...
                          Nano3d::Canvas_rgb565::Texture *dst, int x, int y)
{
	...
	unsigned x_max = Nano3d::min((int)TILE_SIZE, src->w() - x*TILE_SIZE);
	unsigned y_max = Nano3d::min((int)TILE_SIZE, src->h() - y*TILE_SIZE);
...
class Banner
{
	...
	public:
		...
		:
			...
			_nitpicker(_width + TILE_SIZE, 2*(_height + TILE_SIZE),
			           true, Framebuffer::Mode::RGB565, true),
