...
static unsigned mousevalue(Input::Event * ev)
{
	/* bit 3 is always set */
	unsigned ret = 0x8;
...
void Vancouver_console::entry()
{
	...
	Chunky_canvas<Pixel_rgb565> canvas((Pixel_rgb565 *) _pixels, Area(_fb_mode.width(), _fb_mode.height()));
	...
	while (1) {
		while (!input.is_pending()) {
			...
			if (_regs && ++count % 10 == 0 && _regs->mode == 0 && _guest_fb && !revoked && fb_active) {
...
Vancouver_console::Vancouver_console(Motherboard &mb, Genode::size_t vm_fb_size, Genode::Dataspace_capability fb_ds)
: _vm_fb_size(vm_fb_size), _mb(mb), _fb_size(0), _pixels(0), _guest_fb(0), _regs(0), _fb_ds(fb_ds)
{
	start();
}
