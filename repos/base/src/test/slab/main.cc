...
int main(int argc, char **argv)
{
...

	{
		...
		for (unsigned i = 1; i <= 10; i++) {
			printf("round %u (used quota: %zd, time: %ld ms)\n",
			       i, alloc.consumed(), timer.elapsed_ms());

			Array_of_slab_elements array(slab, i*100000, SLAB_SIZE);
			printf(" allocation completed (used quota: %zd", alloc.consumed());
		}

		printf(" finished (used quota: %zd, time: %ld ms)\n",
	...
	printf("destructed slab (used quota: %zd)\n", alloc.consumed());
	if (alloc.consumed() > 0) {
