...
content: README
README:
	cp $(REP_DIR)/recipes/src/base-sel4-imx8q_evk/README $@
...
etc/board.conf:
	echo "BOARD = imx8q_evk" > etc/board.conf

content:
	mv lib/mk/spec/arm_v8a/ld-sel4.mk lib/mk/spec/arm_v8a/ld.mk
	cp -r $(GENODE_DIR)/repos/imx/src/timer/gpt src/timer/gpt
	sed -i "s/imx8_timer/timer/" src/timer/gpt/target.mk
	find lib/mk/spec -name kernel-sel4-*.mk -o -name syscall-sel4-*.mk |\
		grep -v "sel4-imx8q_evk.mk" | xargs rm -rf
