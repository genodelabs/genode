...
class Virtio_nic::Session_component : public Nic::Session_component
{
	...
	public:
		...
		try : Nic::Session_component(tx_buf_size, rx_buf_size, Genode::CACHED,
		      ...
		      _device(env, platform_device.io_mem_dataspace(), io_mem_offset),
		...
		Session_component *_create_session(const char *args) override
		{
			...
			try {
				return new (md_alloc()) Session_component(
					_env, _device, _device_io_offset, *md_alloc(), rom.xml(), tx_buf_size, rx_buf_size);
			} catch (...) { throw Service_denied(); }
		}

		Platform::Device_capability _find_device()
		{
			using String = Genode::String<64>;

			Platform::Device_capability cap;

			_platform.with_xml([&] (Xml_node const &xml) {
				xml.for_each_sub_node("device", [&] (Xml_node const &device_node) {
					...
					cap = _platform.acquire_device(name.string());
					_device_io_offset = offset;
				});
			});

			if (!cap.valid()) {
				Genode::error("No VirtIO GPU device found!");
