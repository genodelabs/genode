...
class Gpio::Reg : Attached_io_mem_dataspace , Mmio
{
	struct Regs : Genode::Mmio
	{
		...
		void setCon(unsigned int con){ write<Con>(con);	}
		void setDat(unsigned int dat){ write<Dat>(dat);	}
		unsigned int getCon(){ return read<Con>();}
		unsigned int getDat(){ return read<Dat>();}
	};

	struct IrqRegs : Genode::Mmio
	{
		...
		IrqRegs(Genode::addr_t base) :Genode::Mmio(base){}

		void enableTriggers(unsigned gpio,unsigned value)
		{
			write<Int_Con>(0);
			switch(gpio)
			{
...
public:
	Reg(addr_t base, size_t size):Attached_io_mem_dataspace(base, size), Mmio((addr_t)local_addr<Reg>()) { }

	void setDirection(int gpio, bool input, Genode::off_t offset)
	{
	...
	void writePin(unsigned gpio, bool level, Genode::off_t offset)
	{
		Regs _reg((Genode::addr_t)local_addr<void>() + offset);
		unsigned int value;
		value = _reg.getDat();
	...
	bool readPin(unsigned gpio, Genode::off_t offset)
	{
		Regs _reg((Genode::addr_t)local_addr<void>() + offset);
		return (_reg.getDat() & (1 << gpio)) !=0;
	}

	void setEnableTriggers(unsigned gpio, Genode::off_t offset,unsigned value){
		IrqRegs _irqRegs((Genode::addr_t)local_addr<void>() + offset);
		_irqRegs.enableTriggers(gpio,value);
...
const Genode::off_t _bankOffset [MAX_BANKS]=
{
		/*Part1*/
		/*GPA0    GPA1     GPB    GPC0    GPC1    GPD0    GPD1    GPF0    GPF1    GPF2    GPF3    ETC1    GPJ0    GPJ1*/
...
const Genode::off_t _irqOffset [MAX_BANKS]=
{
		/*Bank 1 irq*/
