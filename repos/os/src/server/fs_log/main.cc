	...
	protected:
		...
		Session_component *_create_session(const char *args)
		{
			...
			*dir_path = '/';
			...
			char label[Log_session::String::MAX_SIZE];
			try {
				...
				if (merge) {
					 ...
					 */
					policy.attribute("label").value(label, sizeof(label));
					if (!*label) {
						PERR("cannot merge an empty policy label");
						throw Root::Unavailable();
					}
					strncpy(dir_path+1, label, MAX_PATH_LEN-1);
				} else
					strncpy(dir_path+1, session_label.string(), MAX_PATH_LEN-1);
			...
			if (*label)
				return new (md_alloc()) Labeled_session_component(label, *file);
