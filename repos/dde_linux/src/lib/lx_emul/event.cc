...
/*
 * Input keycodes duplicated from input/keycodes.h as Genode enums and Linux
 * macro definitions collide on direct inclusion.
 */
...
enum Keycode {
#define GENODE_TAG_VALUE(tag, value) G_ ## tag = value,
...
unsigned lx_emul_event_keycode(unsigned linux_keycode)
{
	switch (linux_keycode) {
#define GENODE_TAG_VALUE(tag, value) case tag: return G_ ## tag;
		GENODE_INPUT_KEYCODES
#undef GENODE_TAG_VALUE

	default:
		Genode::warning("unknown keycode ", linux_keycode);
	}
